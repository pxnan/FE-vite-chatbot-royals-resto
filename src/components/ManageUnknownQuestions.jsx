...
const ManageUnknownQuestions = ({ apiBaseURL, onDataChange }) => {
    ...
    const fetchData = async (page = 1) => {
        ...
        setError('');
        try {
            const response = await fetch(`${apiBaseURL}/pertanyaan-unknown?page=${page}&per_page=20`);
    ...
    const handleDeleteConfirm = async () => {
        try {
            const response = await fetch(`${apiBaseURL}/delete-unknown`, {
                method: 'DELETE',
    ...
    const handleDeleteAll = async () => {
        try {
            const response = await fetch(`${apiBaseURL}/delete-all-unknown`, {
                method: 'DELETE'
            });
            ...
            if (response.ok) {
                ...
                setError(result.error || 'Gagal menghapus semua data');
            }
        } catch (err) {
            setError('Terjadi kesalahan jaringan');
        }
    ...
    const handleDeleteSelected = async () => {
        for (const id of selectedItems) {
            await fetch(`${apiBaseURL}/delete-unknown`, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
