...
const ManageDataset = ({ apiBaseURL, onDataChange }) => {
    ...
    const fetchData = async (page = 1, searchTerm = '', kategori = '') => {
        ...
        setError('');
        try {
            let url = `${apiBaseURL}/get-all-data?page=${page}&per_page=20`;
    ...
    const handleUpdate = async () => {
        if (!editForm.pertanyaan.trim() || !editForm.jawaban.trim() || !editForm.kategori.trim()) {
            setError('Semua field harus diisi');
            return;
        }
        
        try {
            const response = await fetch(`${apiBaseURL}/update-data`, {
                method: 'PUT',
    ...
    const handleDeleteConfirm = async () => {
        try {
            const response = await fetch(`${apiBaseURL}/delete-data`, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
                ...
                <div className="card-body">
                    <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
                        <h2 className="card-title text-xl">
                            Kelola Dataset
                            <div className="badge badge-info ml-2">{totalData} Data</div>
                        </h2>
                    ...
                    {data.length === 0 ? (
                        ...
                        <div className="overflow-x-auto">
                            <table className="table table-zebra w-full">
                                ...
                                <tbody>
                                    {data.map((item, idx) => (
                                        <tr key={item.index || idx} className="hover:bg-base-200 transition-colors">
                                            <td className="font-medium">{(currentPage - 1) * 20 + idx + 1}</td>
                                            <td className="whitespace-normal break-words max-w-md">
                                                {editingId === item.index ? (
                                                    ...
                                                    <div className="group relative pr-8">
                                                        ...
                                                        <button
                                                            onClick={() => handleCopyQuestion(item.pertanyaan)}
                    ...
                    <div className="text-center text-xs text-gray-500 mt-4">
                        Menampilkan {((currentPage - 1) * 20) + 1} - {Math.min(currentPage * 20, totalData)} dari {totalData} data
                    </div>
                </div>
            </div>
            ...
            {showDeleteModal && (
                <div className="modal modal-open">
                    <div className="modal-box">
                        ...
                        <p className="py-4">Apakah Anda yakin ingin menghapus data ini? Tindakan ini tidak dapat dibatalkan.</p>
