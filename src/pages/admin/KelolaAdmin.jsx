...
const KelolaAdmin = () => {
    ...
    const fetchAdmins = async (page = 1, searchTerm = '') => {
        ...
        try {
            ...
            let url = `${apiBaseURL}/api/admins?page=${page}&per_page=10`;
    ...
    const handleEditAdmin = async () => {
        ...
        try {
            const token = getToken();
            const response = await fetch(`${apiBaseURL}/api/admins/${selectedAdmin.id}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
    ...
    const handleResetPassword = async () => {
        ...
        try {
            const token = getToken();
            const response = await fetch(`${apiBaseURL}/api/admins/${selectedAdmin.id}/reset-password`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
    ...
    const handleDeleteAdmin = async () => {
        try {
            const token = getToken();
            const response = await fetch(`${apiBaseURL}/api/admins/${deleteId}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
